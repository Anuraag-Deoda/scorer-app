...
function ScoreboardContent({
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow className="border-b-muted/50">
                    <TableHead className="w-[35%] font-semibold text-xs sm:text-sm px-2 sm:px-4">
                      Batter
                    </TableHead>
                    <TableHead className="text-left text-xs sm:text-sm px-2 sm:px-4">
                      Status
                    </TableHead>
                    <TableHead className="text-right font-semibold text-xs sm:text-sm px-2 sm:px-4">
                      R
                    </TableHead>
                    <TableHead className="text-right text-xs sm:text-sm px-2 sm:px-4">
                      B
                    </TableHead>
                    <TableHead className="text-right text-xs sm:text-sm px-2 sm:px-4">
                      4s
                    ...
                    .map((player: Player) => {
                      ...
                      return (
                        ...
                        >
                          <TableCell className="font-medium py-1 px-2 sm:px-4">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1">
                              ...
                              {player.isImpactPlayer && (
                                <Badge
                                  variant="outline"
                                  className="text-xs px-1 py-0.5 font-normal"
                                >
                                  IP
                                </Badge>
                              )}
                          ...
                          <TableCell className="text-right py-1 px-2 sm:px-4">
  ...
  const BowlingCard = ({ innings }: { innings: Innings }) => {
    const bowlers = innings.bowlingTeam.players.filter(
      (p: Player) =>
        (!p.isSubstitute || p.isImpactPlayer) && p.bowling.ballsBowled > 0
    );
          ...
          <div className="min-w-full inline-block align-middle">
            <div className="overflow-hidden">
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow className="border-b-muted/50">
                    <TableHead className="w-[40%] font-semibold text-xs sm:text-sm px-2 sm:px-4">
                      Bowler
                    </TableHead>
                    <TableHead className="text-right font-semibold text-xs sm:text-sm px-2 sm:px-4">
                      O
                ...
                <TableBody>
                  {bowlers.map((player: Player) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right py-1 px-2 sm:px-4">
                        {player.bowling.economyRate.toFixed(2)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
    ...
    <div className="bg-card/50 rounded-md p-2 sm:p-3 space-y-2">
      ...
      {innings.fallOfWickets.map((fow: any, index: number) => (
        ...
        >
          ...
          ({fow.playerOut}, {fow.over.toFixed(1)} ov)
        </p>
      ))}
    </div>
  );
    ...
    return (
       <div className="p-2 sm:p-4 space-y-3 sm:space-y-4 bg-muted/40">
        {sortedOvers.map((overNum) => {
          const bowlerId = ballsByOver[overNum][0]?.bowlerId;
          const bowler = innings.bowlingTeam.players.find(
            (p) => p.id === bowlerId
          );
          return (
            <div
              key={overNum}
              className="flex flex-col sm:flex-row sm:items-center gap-2 bg-card rounded-md p-2 sm:p-3"
            >
              <p className="text-sm text-muted-foreground w-12 font-semibold text-center sm:text-left">
                Ov {overNum + 1}
              </p>{" "}
              {bowler && (
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                {bowler && (
                  <p className="text-sm text-muted-foreground w-28 truncate">
                    {bowler.name}
                  </p>
                )}
              </div>
                ...
                {ballsByOver[overNum].map((ball, ballIndex) => (
                  <span
                    ...
                    className={`flex items-center justify-center h-5 w-5 sm:h-6 sm:w-6 rounded-full font-bold text-xs border
                                ...
                                ${
                                  ["wd", "nb", "lb", "b"].includes(ball.event)
                                    ? "bg-yellow-500 text-yellow-950 border-yellow-500"
                                    : ""
                                } /* Changed extra colors */
                                ${
                                  ball.runs === 4
                                ...
                                ${
                                  ...
                                  ball.runs < 4
                                    ...
                                    : ""
                                } /* Added border to runs */
                                ${
                                  ball.event === "run" && ball.runs === 0
                                    ? "bg-muted/50 border-muted/50"
                                    : ""
                                } /* Added border to dots */
                            `}
                  >
                    {ball.display}
                  </span>
  ...
  const BowlerSelection = () => {
    const availableBowlers = currentInnings.bowlingTeam.players.filter(
      (p) => !p.isSubstitute || p.isImpactPlayer
    );
    ...
    return (
      <div className="p-6 space-y-4 text-center bg-muted/40 rounded-b-lg">
        ...
        <p className="text-muted-foreground text-sm">
          Choose the player to bowl the next over.
        </p>
  ...
  const RunRateChart = () => {
    const getOverByOverScores = (innings: Innings) => {
      ...
      for (let i = 0; i < maxOvers; i++) {
        const ballsInThisOver = innings.timeline.filter(
          (b) => Math.floor(b.over ?? 0) === i
        );
        const runsInThisOver = ballsInThisOver.reduce(
          (acc, ball) => acc + ball.runs + ball.extras,
          0
        );
        score += runsInThisOver;
        const legalBallsThisOver = ballsInThisOver.filter(
    ...
    const chartData = Array.from(
      { length: Math.max(innings1Scores.length, innings2Scores.length) },
      (_, i) => ({
        over: i + 1,
  ...
  const WormGraph = () => {
    const getOverByOverData = (innings: Innings) => {
      ...
      for (let i = 0; i < maxOvers; i++) {
        ...
        const wicketsInThisOver = ballsInThisOver.filter(
          (b) => b.isWicket
        ).length;
    ...
    const chartData = Array.from(
      { length: Math.max(innings1Data.length, innings2Data.length) },
      (_, i) => {
        ...
        return {
          ...
          [`${innings1.battingTeam.name}_wickets`]: innings1.timeline.filter(
            (b) => b.isWicket && Math.floor(b.over ?? 0) === i
          ),
          [`${innings2 ? innings2.battingTeam.name : ""}_wickets`]: innings2
            ? innings2.timeline.filter(
                (b) => b.isWicket && Math.floor(b.over ?? 0) === i
              )
            : [],
        };
        ...
        <ChartContainer config={chartConfig} className="w-full h-full">
          ...
          >
            ...
            <defs>
              ...
              {innings2 && (
                <linearGradient id="fillWormTeam2" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
                    stopColor="hsl(var(--chart-2))"
                    stopOpacity={0.8}
  ...
  const showBowlerSelection =
    currentInnings.currentBowler === -1 &&
    match.status === "inprogress" &&
    currentInnings.overs < match.oversPerInnings;
      ...
      <CardContent className="p-0 bg-muted/40 rounded-lg w-full max-w-full overflow-hidden">
        ...
        {matchSituation.isChasing && (
          <div className="p-2 sm:p-3 text-center bg-primary/10">
            {match?.result ? (
              ...
              <p className="font-semibold text-base sm:text-lg">
                {`${matchSituation.battingTeamName} need ${matchSituation.runsNeeded} run in ${matchSituation.ballsRemaining} balls to win.`}
              </p>
            )}
          </div>
        )}
        {showBowlerSelection && !isSimulating && (
          <div className="p-4 text-center bg-yellow-50 border-b border-yellow-200">
            <h4 className="font-semibold text-lg text-yellow-800 mb-2">
              Select Next Bowler
            </h4>
            ...
            <TooltipProvider>
              <ShadTooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger
                    value="scoreboard"
                    className="rounded-none rounded-tl-lg min-w-0 px-2 sm:px-3 text-xs sm:text-sm"
                  >
              ...
              <ShadTooltip>
                <TooltipTrigger asChild>
                  <TabsTrigger
                    value="summary"
                    className="rounded-none min-w-0 px-2 sm:px-3 text-xs sm:text-sm"
                  >
              ...
              {match.currentInnings > 1 && (
                <ShadTooltip>
                  <TooltipTrigger asChild>
                    <TabsTrigger
                      value="winprob"
                      className="rounded-none rounded-tr-lg min-w-0 px-2 sm:px-3 text-xs sm:text-sm"
                    >
              ...
              {match.currentInnings === 1 && (
                <TabsTrigger
                  value=""
                  className="rounded-none rounded-tr-lg invisible min-w-0"
                ></TabsTrigger>
              )}
            </TooltipProvider>
          </TabsList>
          ...
          <TabsContent value="scoreboard" className="p-2 sm:p-3 space-y-3">
            <Tabs defaultValue="innings1" className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-card rounded-md">
                <TabsTrigger
                  value="innings1"
                  className="rounded-md data-[state=active]:bg-primary/10 text-xs sm:text-sm"
                >
                  {innings1.battingTeam.name}
                </TabsTrigger>
                {innings2 && (
            ...
            <Tabs defaultValue="innings1" className="w-full">
              ...
              <TabsContent value="innings1" className="p-0 pt-3 space-y-3">
                {innings1.fallOfWickets.length > 0 ? (
                  <FallOfWickets innings={innings1} />
                ) : (
              ...
              {innings2 && (
                <TabsContent value="innings2" className="p-0 pt-3 space-y-3">
                  {innings2.fallOfWickets.length > 0 ? (
                    <FallOfWickets innings={innings2} />
                  ) : (
                    <p className="p-4 text-center text-muted-foreground text-sm">
                      No wickets have fallen yet.
                    </p>
