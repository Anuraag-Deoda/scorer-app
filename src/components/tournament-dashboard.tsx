...
export default function TournamentDashboard({
  ...
  useEffect(() => {
    ...
    const matchesByRound = Object.fromEntries(
      playoffRounds.map((r) => [
        r,
        tournament.matches.find((m) => m.round === r),
      ])
    );
    ...
    if (
      ...
      finalMatch &&
      q1.status === "finished" &&
      elim.status === "finished" &&
      (q2.team1Name === "TBD" ||
        q2.team2Name === "TBD" ||
        finalMatch.team1Name === "TBD" ||
        finalMatch.team2Name === "TBD")
    ) {
      ...
      if (q1WinnerId && q1LoserId && elimWinnerId) {
        ...
        const q2Team1Name =
          tournament.teams.find((t) => t.id === q2Team1Id)?.name || "TBD";
        const q2Team2Name =
          tournament.teams.find((t) => t.id === q2Team2Id)?.name || "TBD";

        // For Final, need Q1 winner and Q2 winner (if Q2 finished, else keep TBD)
        const finalTeam1Id = q1WinnerId;
        const finalTeam1Name =
          tournament.teams.find((t) => t.id === finalTeam1Id)?.name || "TBD";
        ...
        if (q2.status === "finished" && q2.winnerTeamId) {
          ...
          finalTeam2Name =
            tournament.teams.find((t) => t.id === finalTeam2Id)?.name || "TBD";
        } else {
          finalTeam2Id =
            q2Team1Id && q2Team2Id ? undefined : finalMatch.team2Id;
          finalTeam2Name = "TBD";
        }

        // Only update if values have changed
        if (
          q2.team1Id !== q2Team1Id ||
          q2.team2Id !== q2Team2Id ||
          q2.team1Name !== q2Team1Name ||
          q2.team2Name !== q2Team2Name ||
          finalMatch.team1Id !== finalTeam1Id ||
          finalMatch.team1Name !== finalTeam1Name ||
          finalMatch.team2Id !== finalTeam2Id ||
          finalMatch.team2Name !== finalTeam2Name
        ) {
          const updatedMatches = tournament.matches.map((m) => {
            if (m.id === q2.id) {
              return {
        ...
        match.matchData.innings.forEach((innings, index) => {
          ...
          battingTeam.players.forEach((player) => {
            ...
            if (!playerStat) {
              ...
              // console.log(
              //   `        Created new player stat for ${player.name} in team ${tournamentTeam.name}`
              // );
            }
            ...
            // console.log(
            //   `        Updated batting stats: runs=${playerStat.runs}, balls=${playerStat.ballsFaced}`
            // );
          });
          ...
          // console.log(
          //   `    Bowling team: ${bowlingTeam.name} (${bowlingTeam.players.length} players)`
          // );
          ...
          if (!tournamentBowlingTeam) {
            // console.log(
            //   `    WARNING: Could not find tournament team with name "${bowlingTeam.name}"`
            // );
            return;
          }
  ...
  const endMatch = (matchResult: Match) => {
    ...
    const finishedEliminator = updatedMatches.find(
      (m) => m.round === "eliminator" && m.status === "finished"
    );
    const finishedQ1 = updatedMatches.find(
      (m) => m.round === "qualifier1" && m.status === "finished"
    );
    const finishedQ2 = updatedMatches.find(
      (m) => m.round === "qualifier2" && m.status === "finished"
    );
    ...
    if (q2) {
      if (finishedEliminator?.winnerTeamId) {
        ...
        q2.team1Name =
          tournament.teams.find((t) => t.id === finishedEliminator.winnerTeamId)
            ?.name || "TBD";
      }
      if (finishedQ1?.loserTeamId) {
        q2.team2Id = finishedQ1.loserTeamId;
        q2.team2Name =
          tournament.teams.find((t) => t.id === finishedQ1.loserTeamId)?.name ||
          "TBD";
      }
    }
    ...
    if (finalMatch) {
      if (finishedQ1?.winnerTeamId) {
        ...
        finalMatch.team1Name =
          tournament.teams.find((t) => t.id === finishedQ1.winnerTeamId)
            ?.name || "TBD";
      }
      if (finishedQ2?.winnerTeamId) {
        finalMatch.team2Id = finishedQ2.winnerTeamId;
        finalMatch.team2Name =
          tournament.teams.find((t) => t.id === finishedQ2.winnerTeamId)
            ?.name || "TBD";
      }
    }
        ...
        <Card className="border-2 border-orange-200 bg-orange-50 dark:border-orange-700 dark:bg-orange-900/20">
          <CardHeader>
            ...
            <CardDescription className="text-orange-700 dark:text-orange-300">
              This tournament is currently in auction mode. Teams need to bid on
              players before matches can begin.
            </CardDescription>
          </CardHeader>
          <CardContent>
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <div className="text-2xl font-bold text-green-700">
                                      {battingPlayers[0].playerName}
                                    </div>
                                    <div className="text-sm text-green-600">
                                      {battingPlayers[0].runs} runs in{" "}
                                      {battingPlayers[0].matches} matches
                                    </div>
                                  </div>
                                  <div className="text-right">
                                    <div className="text-3xl font-bold text-green-600">
                                      {battingPlayers[0].runs}
                                    </div>
                                    <div className="text-sm text-green-600">
                                      runs
                                    </div>
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <div className="text-2xl font-bold text-purple-700">
                                      {bowlingPlayers[0].playerName}
                                    </div>
                                    <div className="text-sm text-purple-600">
                                      {bowlingPlayers[0].wickets} wickets in{" "}
                                      {bowlingPlayers[0].matches} matches
                                    </div>
                                  </div>
                                  <div className="text-right">
                                    <div className="text-3xl font-bold text-purple-600">
                                      {bowlingPlayers[0].wickets}
                                    </div>
                                    <div className="text-sm text-purple-600">
                                      wickets
                                    </div>
          ...
          <TabsContent value="auction">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Bid on players to build your tournament teams. Each team
                  starts with ₹100 purse and needs 13 players.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <TournamentAuction
                  ...
                  onAuctionComplete={(updatedTeams) => {
                    ...
                    const updatedTournament = {
                      ...
                      status: "draft" as const,
                      teams: tournament.teams.map((team) => {
                        const auctionTeam = updatedTeams.find(
                          (at) => at.id === team.id
                        );
                        return {
                          ...team,
                    ...
                    toast({
                      ...
                      description:
                        "Teams have been built through the auction system.",
                    });
                  }}
